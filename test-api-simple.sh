...
# 토큰 추출
ACCESS_TOKEN=$(echo "$LOGIN_RESPONSE" | jq -r '.data.accessToken' 2>/dev/null || echo "$LOGIN_RESPONSE" | grep -o '"accessToken":"[^"]*"' | cut -d'"' -f4)
...
# 그룹 ID 추출
GROUP_ID=$(echo "$GROUP_RESPONSE" | jq -r '.data.group.id' 2>/dev/null || echo "$GROUP_RESPONSE" | grep -o '"id":"[^"]*"' | head -1 | cut -d'"' -f4)
